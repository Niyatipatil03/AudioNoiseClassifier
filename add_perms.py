...
# 2) Force local.properties to set SDK versions (overrides Flutter defaults)
...
print("local.properties updated (min=26, target=34, compile=34).")

# 3) Enforce SDK versions in android/app/build.gradle
gradle_path = 'android/app/build.gradle'
if os.path.exists(gradle_path):
    with open(gradle_path, 'r', encoding='utf-8') as f:
        gradle_content = f.read()

    # Replace legacy Groovy style
    ...
    # Replace possible Kotlin DSL-like assignments
    ...
    print("build.gradle updated (min=26, target=34, compile=34).")
else:
